...
/*   Updated: 2025/06/12 02:36:11 by aguinea          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_vec(char *str, t_vec *vec)
{
	...
	if (!split || !split[0] || !split[1] || !split[2] || split[3])
		return (free_array(split), printf("Invalid vector format\n"), 0);
...
static int	parse_fov(char *fov_str, t_scene *scene)
{
	...
	if (!is_valid_float(fov_str))
		return (printf("Invalid float (camera[fov])\n"), 0);
	fov = ft_atof(fov_str);
	if (fov < 0.0 || fov > 180.0)
		return (printf("FOV must be in range [0,180]\n"), 0);
...
static int	parse_orientation(t_vec vec)
{
	if (vec.x < -1.0 || vec.x > 1.0
		|| vec.y < -1.0 || vec.y > 1.0
		|| vec.z < -1.0 || vec.z > 1.0)
		return (printf("Camera orientation must be in range [-1,1]\n"), 0);
	return (1);
}
...
static int	parse_camera_orientation(char **tokens, t_scene *scene)
{
	...
	if (!parse_vec(tokens[1], &pos))
		return (printf("Invalid camera position vector\n"), 0);
	if (!parse_vec(tokens[2], &orient))
		return (printf("Invalid camera orientation vector\n"), 0);
...
int	parse_camera(char **tokens, t_scene *scene)
{
	if (!tokens[1] || !tokens[2] || !tokens[3])
		return (printf("Camera requires 3 parameters\n"), 0);
