...
/*   Updated: 2025/07/16 13:02:19 by aguinea          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void check_cylinder_cap(t_ray ray, t_cylinder *cy, t_vec axis, t_hit *hit)
{
    int i;
    double t;
    double denom;
    ...
    while (++i < 2)
    {
        center = (i == 0) ? cy->position : vec_add(cy->position, vec_scale(axis, cy->height));
...
static void intersect_cylinder_body(t_ray ray, t_cylinder *cy, t_cyl_data *d, t_hit *hit)
{
    int i;
    double t;
    double h;
...
static int prepare_cylinder_data(t_ray ray, t_cylinder *cy, t_cyl_data *out)
{
    ...
    out->d_proj = vec_sub(ray.direction, vec_scale(out->axis, vec_dot(ray.direction, out->axis)));
    out->oc_proj = vec_sub(out->oc, vec_scale(out->axis, vec_dot(out->oc, out->axis)));
